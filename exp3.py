...
def loader_transform():
    return transforms.Compose([transforms.Resize((180 // SCALE_FACTOR, 320 // SCALE_FACTOR)), transforms.ToTensor()])

trainset = ImageFolderWithPaths(
    root=os.path.join(directory, "../experiment3/training"),
    transform=loader_transform()
)

testset1 = ImageFolderWithPaths(
    root=os.path.join(directory, "../experiment3/test1"),
    transform=loader_transform()
)

testset2 = ImageFolderWithPaths(
    root=os.path.join(directory, "../experiment3/test2"),
    transform=loader_transform()
)
...
class NNModel(nn.Module):
    def __init__(self):
        ...
        self.fc_a = nn.Linear(image_width * image_height, 120)
        self.fc_b = nn.Linear(image_width * image_height, 120)

        self.fc1 = nn.Linear(2 * 120, 80)
...
acc_line1 = ([], []) # x,y points that will be plotted
acc_line2 = ([], []) # x,y points that will be plotted

acc_line1[0].append(0)
acc_line1[1].append(eval_testset(model, testloader1))

acc_line2[0].append(0)
acc_line2[1].append(eval_testset(model, testloader2))
...
for epoch in range(num_epochs):
    ...
    acc_line1[0].append(epoch + 1)
    acc_line1[1].append(eval_testset(model, testloader1))

    acc_line2[0].append(epoch + 1)
    acc_line2[1].append(eval_testset(model, testloader2))
...
ax2.set_ylim([-5, 105])
ax2.plot(acc_line1[0], acc_line1[1], color=color)
ax2.plot(acc_line2[0], acc_line2[1], color='tab:orange')
